...
flags.DEFINE_string('root_dir', './data3/', 'Root directory for writing '
...
def main_loop(_):
    ...
    if FLAGS.agent != 'ga':
        agent.infer(num_samples=1, mean_action=FLAGS.mean_action, visualize=True,
                    save_video=True, only_road=FLAGS.only_road)
    else:
        best_solution, _ = agent.load_ga()
        _, plan, log_eval = agent.fitness_ga(
            best_solution, num_samples=1,
            mean_action=FLAGS.mean_action, visualize=True,
