...
from ReinforcementLearning.rl.agents import Agent
from ReinforcementLearning.rl.core import estimate_advantages, LoggerRL
...
class UrbanPlanningAgent(Agent):
    ...
    def __init__(self, cfg: Config, dtype: torch.dtype, device: torch.device,
        ...
        self.tau = cfg.tau
        self.opt_num_epochs = cfg.num_optim_epoch
        ...
        self.policy_grad_clip = [(self.policy_net.parameters(),1),(self.value_net.parameters(),1)]
        ...
        with to_test(*self.update_modules):
            with torch.no_grad():
                ...
                for i in range(0, len(states), chunk):
                    states_i = self.tensorfy(states[i:min(i + chunk, len(states))], self.device)
    ...
    def update_policy(self, states, actions, returns, advantages, exps, iteration):
        ...
        for epoch in range(self.opt_num_epochs):
            ...
            for i in range(optim_batch_num):
                ...
                states_b = self.tensorfy(states_b, self.device)
    ...
    def log_optimize_policy(self, iteration, info):
        ...
        tb_logger.add_scalar('train/train_R_wastemgmt_eps_avg', log.avg_episode_wastemgmt_reward, iteration)
        # tb_logger.add_scalar('train/train_R_drainage_eps_avg', log.avg_episode_drainage_reward, iteration)
        ...
        tb_logger.add_scalar('eval/eval_R_wastemgmt_eps_avg', log_eval.avg_episode_wastemgmt_reward, iteration)
        # tb_logger.add_scalar('eval/eval_R_drainage_eps_avg', log_eval.avg_episode_drainage_reward, iteration)
            ...
            with torch.no_grad():
                ...
                while logger.num_steps < num_samples:
                    ...
                    if visualize:
                        self.env.visualize(
                            save_fig=True,
                            path=os.path.join("./video", 'step_land_use_0000.svg'),
                            legend=False,
                            ticks=False,
                            bbox=False)
                    ...
                    for t in range(1, 10000):
                        ...
                        if visualize:
                            if state[-1].argmax() == 0:
                                ...
                                self.env.visualize(
                                    save_fig=True,
                                    path=os.path.join("./video", f'step_land_use_{t:04d}.svg'),
                                    legend=False,
                                    ticks=False,
                                    bbox=False)
                            if next_state[-1].argmax() != 0:
                                self.env.visualize_road_and_boundary(
                                    save_fig=True,
                                    path=os.path.join("./video", f'step_road_{t:04d}.svg'),
                                    legend=False,
                                    ticks=False,
                                    bbox=False)
                            self.env.visualize(
                                save_fig=True,
                                path=os.path.join("./video", f'step_all_{t:04d}.svg'),
                                legend=False,
                                ticks=False,
                                bbox=False)
                        if done:
    ...
    def infer(self, num_samples=1, mean_action=True, visualize=False, save_video=False, only_road=False):
        ...
        if save_video:
            if only_road:
                save_video_ffmpeg(f'./video/step_road_%04d.svg',
                                  f'./video/plan_road.mp4', fps=10)
            else:
                save_video_ffmpeg(f'./video/step_land_use_%04d.svg',
                                  f'./video/plan_land_use.mp4', fps=10)
            save_video_ffmpeg(f'./video/step_all_%04d.svg',
                              f'./video/plan_all.mp4', fps=10)
