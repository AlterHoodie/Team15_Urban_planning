...
flags.DEFINE_string('root_dir', './data2/', 'Root directory for writing '
...
def main_loop(_):
    ...
    env = agent.env
    plc = env._plc
    # plc._add_domain_features()
    gdf = plc._gdf
    env._stage = "done"
    print(env.get_reward_info())
    # print(env._get_all_reward_info())
    reward, dictionary = env._get_all_reward_info()
    print(reward)
    for key, value in dictionary.items():
        print(f"{key}\n{value}")
    
    print(gdf[gdf['geometry'].geom_type=="LineString"]['population'].describe())
    print(gdf)
