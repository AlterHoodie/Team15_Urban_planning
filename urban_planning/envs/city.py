...
def reward_info_function(
    ...
    wastemgmt_weight: float = 1.0,
    drainage_weight: float = 1.0,
    ...
    if name == 'intermediate':
        ...
        if wastemgmt_weight > 0.0:
            wastemgmt = plc.get_wastemgmt_reward()
            proxy_reward += wastemgmt_weight * wastemgmt

        if drainage_weight > 0.0:
            drainage = plc.get_drainage_reward()
            proxy_reward += wastemgmt_weight * drainage
...
class CityEnv:
    ...
    def __init__(self,
        ...
        self._reward_info_fn = partial(reward_info_fn,
                                       ...
                                       wastemgmt_weight=cfg.reward_specs.get('wastemgmt_weight', 1.0),
                                       drainage_weight=cfg.reward_specs.get('drainage_weight', 1.0),
    ...
    def _set_cached_reward_info(self):
        ...
        if not self._frozen:
            ...
            self._cached_wastemgmt_reward = -1.0
            self._cached_drainage_reward = -1.0
    ...
    def _get_all_reward_info(self) -> Tuple[float, Dict]:
        ...
        info = {
            ...
            'wastemgmt': land_use_info['wastemgmt'],
            'drainage': land_use_info['drainage'],
    ...
    def create_drainage(self,action):
        """Find shortest paths in a GeoDataFrame using Dijkstra's algorithm.

        Args:
            gdf (GeoDataFrame): A GeoDataFrame containing LineString geometries.
            starting_point (tuple): The starting point as (x, y) coordinates.

        Returns:
            nodes_gdf (GeoDataFrame): GeoDataFrame of the nodes in the shortest path.
            edges_gdf (GeoDataFrame): GeoDataFrame of the edges in the shortest path.
        """
        G = nx.Graph()
        gdf = self._plc.get_gdf()
        # Add edges from LineStrings in the GeoDataFrame
        for idx, row in gdf[gdf['geometry'].geom_type == 'LineString'].iterrows():
            line = row['geometry']
            coords = list(line.coords)

            # Create edges from the start and end points of the LineString
            for i in range(len(coords) - 1):
                G.add_edge(coords[i], coords[i + 1], weight=line.length)

        lengths, paths = nx.single_source_dijkstra(G, action)

        # Step 2: Create a subgraph of the shortest paths
        shortest_paths_edges = []
        for target in paths:
            if target != action:  # Exclude the starting point itself
                path = paths[target]
                for i in range(len(path) - 1):
                    shortest_paths_edges.append((path[i], path[i + 1]))

        # Create a subgraph from the shortest paths
        shortest_path_graph = nx.Graph()
        shortest_path_graph.add_edges_from(shortest_paths_edges)


        # Convert edges to GeoDataFrame
        edges_geom = [LineString([point1, point2]) for point1, point2 in shortest_path_graph.edges()]
        edges_gdf = GeoDataFrame({'geometry': edges_geom,'type':city_config.DRAINAGE,'existence':True})

        updated_gdf = pd.concat([gdf,edges_gdf])

        return  updated_gdf
    
    def find_drainage_point(self):
        gdf = self._plc.get_gdf()
        waterbodies = gdf[gdf['type'] == city_config.WATERBODY]

        if not waterbodies.empty:
            # Randomly select a waterbody
            random_waterbody = waterbodies.sample(n=1).geometry.values[0]

            # Check if the selected geometry is a Polygon
            if isinstance(random_waterbody, Polygon):
                # Get the vertices of the polygon
                vertices = list(random_waterbody.exterior.coords)
                # Randomly select one vertex
                return random.choice(vertices)
    
        # If no waterbodies are found, look for drainage lines
        drainage_lines = gdf[gdf['type'] == city_config.DRAINAGE]

        if drainage_lines.empty:
            raise ValueError("No drainage lines found.")

        # Randomly select a drainage line
        random_drainage = drainage_lines.sample(n=1).geometry.values[0]

        # Get the points from the LineString
        if isinstance(random_drainage, LineString):
            points = list(random_drainage.coords)
            # Randomly select one point from the drainage line
    ...
    def step(self, action: np.ndarray, logger: logging.Logger) -> Tuple[List, float, bool, dict]:
        ...
        if self._done:
            ...
            info['wastemgmt'] = self._cached_wastemgmt_reward
            info['drainage'] = self._cached_drainage_reward
